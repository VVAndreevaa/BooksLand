(function($) {
    ...
    let $deactivateForm  = jQuery('#contact-form-deactivateForm');
    ...
    let deactivateFormSubmitted = localStorage.getItem('contact-form-deactivateFormSubmitted');
    ...
    function submitForm(deactivate_options, id) {
        ...
        data.push({name: 'action', value: 'contact-form-deactivate'});
        ...
        if (!deactivateFormSubmitted) {
            localStorage.setItem('contact-form-deactivateFormSubmitted', true);
            deactivateFormSubmitted = true;

            jQuery.post('./controller/extension/module/elfsight_contact_form.php', jQuery.param(data)).then(function(result) {
